...
var _makeMovableElement = require('src/setup/make-movable-element');

var _makeMovableElement2 = _interopRequireDefault(_makeMovableElement);

var _makeDropableElement = require('src/setup/make-dropable-element');

var _makeDropableElement2 = _interopRequireDefault(_makeDropableElement);

var _makeSelectableElement = require('src/setup/make-selectable-element');

var _makeSelectableElement2 = _interopRequireDefault(_makeSelectableElement);
...
$(function () {
  var setupElements = function setupElements() {
    ...
    //setup canvas
    (0, _makeDropableElement2.default)("#canvas");
  ...
  var setupElement = function setupElement(element) {
    (0, _makeMovableElement2.default)(element);
    (0, _makeDropableElement2.default)(element);
    $(element).editText();
    (0, _makeSelectableElement2.default)(element, "#canvas");
  };

  var duplicateElement = function duplicateElement() {
    var $canvas = $("body");
  ...
  (function () {
    ...
    $("#changeCanvasSizeDialog").dialog({
      ...
      open: function open(event, ui) {
        ...
        $(this).find('input[name="height"]').val(currentHeight);

        $(this).find('input[name="width"]').val(currentWidth);
      },
      buttons: [{
        text: "OK",
        click: function click() {
          ...
          $container.height($(this).find('input[name="height"]').val());
          $container.width($(this).find('input[name="width"]').val());
          $(this).dialog("close");
        }
      }, {
        text: "Cancel",
    ...
    $('<button title="exports to codepen. »Save« on codepen to get a sharable URL">↗ to Codepen</button>').appendTo("#toolbar").click(function () {
      ...
      var htmlString = htmlSource.clone(). //for coming manipulations, so we don't actually change the original
      ...
      each(function (index, element) {
        ...
        $(element).attr("data-editable-content", oldString.replace(/"/g, "&Prime;").replace(/'/g, "&prime;"));
      }).html();

      var cssString = cssSource.html();
      ...
      var form = '<form id="sendToCodepen" action="http://codepen.io/pen/define" method="POST" target="_blank" style="display:none;">' + '<input type="hidden" name="data" value=\'' + jsonstring + '\'>' + '</form>';
  ...
  //loader functions
  var readStringToJquery = function readStringToJquery(string) {
  ...
  function saveDocumentCode() {
    var padNumberWith0es = function padNumberWith0es(number, padding) {
      //number : the number you want to pad with 0es
      //padding: the length of the output string, 5 is 00000 and 00125 etc.

      if (padding + 1 < ("" + number).length) {
        //make padding at last as long as the number, so nothing is cut off.
...
},{"src/setup/make-dropable-element":2,"src/setup/make-movable-element":3,"src/setup/make-selectable-element":4}],2:[function(require,module,exports){
"use strict";
...
exports.default = function (element) {
  ...
  $(element).children(".ui-resizable-handle"). //find handles, which are direct decendants...
  remove(); //remove them (not useful when displaying)
...
exports.default = function (element, selectorCanvasParam) {
  var $element = $(element);
  ...
  $element.mousedown(function (event) {
    if ($(event.target).closest(elementSelector)[0] === $element[0]) {
      //either it is the same element that was clicked, or the element is the clicked element’s the first parent that is a mock element.
